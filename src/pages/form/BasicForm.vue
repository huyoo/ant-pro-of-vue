    ...
    <a-form :form="form" layout="horizontal" @submit="handleSubmit">
      ...
      <a-form-item label="起止日期" :wrapper-col="wrapperCol" :label-col="labelCol">
        <a-range-picker style="width: 100%"
                        v-decorator="['beginDate',{ rules: [{ type: 'array', required: true, message: '请选择起止日期' }]}]"></a-range-picker>
      </a-form-item>
      ...
      <a-form-item
        ...
        <a-textarea
          rows="4"
          placeholder="请输入衡量标准"
          v-decorator="['standard',{rules: [{ required: true, message: '请输入衡量标准' }]}]"/>
      </a-form-item>
      <a-form-item :wrapper-col="wrapperCol" :label-col="labelCol">
        <span slot="label">客户
          <span class="tips">(选填)</span>
          <a-tooltip title="目标的服务对象"><a-icon type="question-circle-o"/></a-tooltip>
        </span>
